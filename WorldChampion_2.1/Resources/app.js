...
//	checkGid();
//		myChess.JDB.setObj(pdata);
//		myChess.JDB.saveFile("playyyyer.json");
...
/*				
for(pi=0;pi<myChess.Players.length;pi++){
...
function checkGid(){
//Ti.API.info("check gid --"+myChess.Players.length);
    var plen = myChess.Players.length;
    
    for(var p=0; p<plen; p++){
        Ti.API.info("Player:"+p+"  "+myChess.Players[p][6]);
        if(myChess.Players[p][7] != (p+1)){
            Ti.API.error("Player index error:"+p+"  "+myChess.Players[p][7]);
            continue;
        }    
        var games = myChess.Games[p];
        var g=0;
        for(g=0; g<games.games.length; g++){
            var game = games.games[g];
            if((g+1)!=game.Gid%10000){
                Ti.API.error("Game Index error:"+no+"/"+game.Gid);
            }
        }
//        Ti.API.info("Finished -- "+g);
    }               
}    
