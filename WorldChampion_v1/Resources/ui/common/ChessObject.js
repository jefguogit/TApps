	...
	for (i = 0; i < 8; ++i) {
      ...
      for (j = 0; j < 8; ++j) {
      	...
      	var aGrid = Ti.UI.createView({
      		...
      		left:j*gridSize,
      		top:i*gridSize,
      		pleft:j*gridSize+gridSize/8,
      		ptop:i*gridSize+gridSize/8,
			left1:j*gridSize,
			top1:i*gridSize,
			left2:j*gridSize+3,
			top2:i*gridSize+3,			
...
//Ti.API.info(type+"-"+no);
	...
	//		setaPiece(pf,pr,aPiece);
			...
			myChess.chessObj.boardPieces[type][no]['picType'] = myChess.chessObj.boardPieces[type][no]['obj'].picType;
...
//Ti.API.info("PIEce:"+piece+"  ("+playerIndex+":"+pieceToMoveIndex+")");
...
function moveAPiece(newMove){
	...
	if((index<0)||(index<100 && index>17)){
		Ti.UI.info("Error: "+newMove+"   "+index);
		return;
	}
...
//		notation.fireEvent('show',{index:max});
		notation.fireEvent('show',{index:no+7});
	}
}
...
myChess.f.emailPGN = function(){
	...
	emailDialog.subject = myChess.chessObj.game.White+" vs "+myChess.chessObj.game.Black+" (PNG from Chess World Champions)";
