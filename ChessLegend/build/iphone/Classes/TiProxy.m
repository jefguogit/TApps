...
//Rare exceptions to indicate a bug in the _chesslegend code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_chesslegend = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_chesslegend]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.chesslegend.module"
