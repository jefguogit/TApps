...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kAppceleratorAPIAPIUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kAppceleratorAPIAPIUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
