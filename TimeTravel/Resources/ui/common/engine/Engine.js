...
//Ti.API.info(i+":"+moves[i]+"    "+this.FormatMove(moves[i]))	;    	
	    ...
	    alert("Error" + moveString + " fen:" + this.GetFen());
	};
	
	self.GetMoveFromSAN = function(moveString) {
Ti.API.info("GetMoveFromSAN:"+moveString);		
	    var moves = this.GenerateValidMoves();
	    for (var i = 0; i < moves.length; i++) {
if(moveString=='Qg2'){
Ti.API.info(i+":"+moves[i]+"    "+this.GetMoveSAN(moves[i]))	;    	
}
	        if (this.GetMoveSAN(moves[i]) == moveString) {
	            return moves[i];
	        }
	    }
	    alert("Error" + moveString + " fen:" + this.GetFen());
...
Ti.API.info("g_toMove:"+this.g_toMove+"    fen:"+fen);	    
...
//Ti.API.info("<<<< "+move+"  "+this.GetMoveSAN(move)+"  "+from+" "+to+" "+captured+" "+piece+"  "+me);
...
//Ti.API.info('ExposesCheck == '+kingPos+"  "+from);	    
    ...
    try{
		...
		Ti.API.info("Error: [IsSquareAttackableFrom] "+e);
	}    
	    return false;
	};
...
//	    this.UpdatePVDisplay(pvMsg);
	...
	self.FinishPlay = function(bestMove) {
	    //postMessage("pv " + BuildPVMessage(bestMove, value, timeTaken, ply));
//Ti.API.info("bestMove --- "+bestMove);	
		var movSan = this.GetMoveSAN(bestMove);   
